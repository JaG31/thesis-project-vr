...
public class FileBrowserTest : MonoBehaviour
{
	...
	{
		TextMesh text = canvasGameObject.AddComponent<TextMesh>();
	...
	void Start()
	{
		...
		FileBrowser.AddQuickLink( "Downloads", $"C:\\Users\\{Environment.UserName}\\Downloads", null );
	...
	void Update() {
		...
		if( FileBrowser.Success && stop == false)
		{
			// Print paths of the selected files (FileBrowser.Result) (null, if FileBrowser.Success is false)
			for( int i = 0; i < FileBrowser.Result.Length; i++ )
				file = FileBrowser.Result[i].ToString();
